...
fn layout_text(s: &str) -> RenderedBlock {
    // TeX treats ordinary Latin variables as math italic.  Keep this at the
    // AST text boundary so operators (`\\sin`, `\\lim`, …) and explicit
    // `\\text{...}` blocks retain their upright forms.
    RenderedBlock::from_text(&mathfont::map_italic_str(s))
}
...
fn layout_supsub(base: &EqNode, sup: &EqNode, sub: &EqNode) -> RenderedBlock {
    // A simultaneous superscript and subscript must occupy distinct rows.
    // Appending Unicode `²ᵢ` to the baseline (the old fast path) changes the
    // meaning and makes the two scripts collide visually.
    let base_block = layout(base);
    let sup_block = layout(sup);
    let sub_block = layout(sub);
...
fn layout_sqrt(body: &EqNode) -> RenderedBlock {
    ...
    if body_h == 1 {
        ...
        // Start the vinculum with a diagonal hook so it visibly joins the
        // radical instead of floating one cell to the right of it.
        ...
        // Start the vinculum with a diagonal hook so it visibly joins the
        // vertical radical stem below it.
        let mut top = vec!["╱".to_string()];
        top.extend(std::iter::repeat_n("─".to_string(), body_w));
        rows.push(top);
...
fn layout_limit(name: &str, lower: &Option<Box<EqNode>>) -> RenderedBlock {
    ...
    if let Some(low) = lower {
        ...
        // TeX math operators are upright and separated from the following
        // operand by operator spacing (`\\sin x`, `\\log n`, …).
        name_block.beside(&RenderedBlock::from_char(' '))
    }
}
